(function( window, $, undefined ) {
	...
	$.elastislide.prototype 	= {
      ...
      var arrow = $('.arrow');
			this.$navNext	= $('.es-nav-next').html(arrow.clone());
			this.$navNext.prependTo(this.$el);
			this.$navPrev	= $('.es-nav-prev').html(arrow.clone());
			this.$navPrev.prependTo(this.$el);
        ...
        if(!$('.es-nav-next').hasClass('disabled')){
          instance._slide('right');
        }
			});
			
			this.$navPrev.on('click.elastislide', function( event ) {
        if(!$('.es-nav-prev').hasClass('disabled')){
